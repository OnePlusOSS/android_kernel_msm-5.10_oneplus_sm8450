...
{
    unsigned int ven_gpio = hw_res->ven_gpio;
    if (gpio_get_value(ven_gpio) != state) {
        TMS_DEBUG("Ven gpio %d state = %d\n", ven_gpio, state);
        gpio_set_value(ven_gpio, state);
    } else {
