				     ...
				     int *vol_mv)
{
	...
	if (unlikely(data != NULL)) {
		...
		if (!oplus_chg_ic_debug_data_check(buf, data->size, batt_num))
			goto skip_overwrite;
...
static int oplus_virtual_gauge_probe(struct platform_device *pdev)
{
	...
	sprintf(chip->ic_dev->manu_name, "virtual gauge\0");
	sprintf(chip->ic_dev->fw_id, "0x00\0");
	chip->ic_dev->type = ic_type;
	chip->ic_dev->get_func = oplus_chg_vg_get_func;
	chip->ic_dev->virq_data = oplus_chg_vg_virq_table;
	chip->ic_dev->virq_num = ARRAY_SIZE(oplus_chg_vg_virq_table);
#ifdef CONFIG_OPLUS_CHG_IC_DEBUG
