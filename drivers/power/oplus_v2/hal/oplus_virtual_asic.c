...
#include "../vooc_ic/oplus_hal_vooc.h"
...
static int oplus_chg_va_init(struct oplus_chg_ic_dev *ic_dev)
{
	...
	/*
	 * Initialization of the child IC will call the interface of the
	 * parent IC, so you need to set the parent IC to online first.
	 */
	ic_dev->online = true;
...
static int oplus_chg_va_reset_sleep(struct oplus_chg_ic_dev *ic_dev)
{
	...
	if (oplus_vooc_btb_temp_over() || oplus_chg_va_is_upgrading(va)) {
		chg_debug("upgrade=%d, btb_temp_over=%d\n",
			  oplus_chg_va_is_upgrading(va),
			  oplus_vooc_btb_temp_over());
		return 0;
	}
					...
					int mode)
{
	...
	if (oplus_vooc_btb_temp_over()) {
		chg_info("btb temp over\n");
		return -EINVAL;
	}
...
static void oplus_va_asic_data_irq_init(struct oplus_virtual_asic_ic *chip)
{
	...
	if (node) {
		...
		asic_gpio->data_irq = CUST_EINT_MCU_AP_DATA;
	}
#else
	asic_gpio->data_irq = gpio_to_irq(asic_gpio->data_gpio);
#endif
...
static int oplus_virtual_asic_probe(struct platform_device *pdev)
{
	...
	sprintf(chip->ic_dev->manu_name, "virtual asic\0");
	sprintf(chip->ic_dev->fw_id, "0x00\0");
	chip->ic_dev->type = ic_type;
	chip->ic_dev->get_func = oplus_chg_va_get_func;
	chip->ic_dev->virq_data = oplus_va_virq_table;
	chip->ic_dev->virq_num = ARRAY_SIZE(oplus_va_virq_table);
