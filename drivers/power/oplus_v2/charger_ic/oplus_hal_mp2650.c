...
static int mp2650_set_icl(struct oplus_chg_ic_dev *ic_dev, int icl_ma)
{
	int tmp;
	...
	chg_debug("usb input max current limit=%d\n", icl_ma);
	tmp = icl_ma - REG00_MP2650_1ST_CURRENT_LIMIT_OFFSET;
	tmp = tmp / REG00_MP2650_1ST_CURRENT_LIMIT_STEP;
...
static int mp2650_get_pre_icl_index(struct chip_mp2650 *chip)
{
	int reg_val = 0;
	int icl_index = 0;
	int rc = 0;
	...
	chg_info("pre icl=%d setting %02x\n",
...
static int mp2650_curr_drop(struct oplus_chg_ic_dev *ic_dev)
{
	...
	pre_icl_index = mp2650_get_pre_icl_index(chip);
	if (pre_icl_index < 2) {
			       ...
			       const struct i2c_device_id *id)
{
	...
	chg_ic->ic_dev =
		devm_oplus_chg_ic_register(chg_ic->dev, node->name, ic_index);
	if (!chg_ic->ic_dev) {
		ret = -ENODEV;
	...
	chg_err("register %s\n", node->name);
	sprintf(chg_ic->ic_dev->manu_name, "buck/boost-mp2762");
	sprintf(chg_ic->ic_dev->fw_id, "0x00");
	chg_ic->ic_dev->type = ic_type;
	chg_ic->ic_dev->get_func = oplus_chg_get_func;
