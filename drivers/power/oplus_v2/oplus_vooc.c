...
{
	if (!g_vooc_chip || !g_vooc_chip->vops || !g_vooc_chip->vops->set_switch_mode) {
		chg_err("  g_vooc_chip is NULL\n");
	} else {
...
static void check_charger_out_work_func(struct work_struct *work)
{
	...
	if (chg_vol >= 0 && chg_vol < 2000) {
		if (chip->vops && chip->vops->reset_fastchg_after_usbout)
...
static void vooc_watchdog_work_func(struct work_struct *work)
{
	...
	oplus_chg_clear_chargerid_info();
	if (chip->vops && chip->vops->set_switch_mode)
...
void fw_update_thread(struct work_struct *work)
{
	...
	if(chip->vooc_fw_update_newmethod) {
		...
		if(!ret) {
			...
			if(chip->vops && chip->vops->fw_check_then_recover) {
...
void oplus_vooc_early_init(struct oplus_vooc_chip *chip)
{
	chip->detach_unexpectly = false;
	chip->allow_reading = true;
	oplus_gauge_unlock();
...
{
	if (!g_vooc_chip || !g_vooc_chip->vops) {
		return;
	} else {
...
{
	if (!g_vooc_chip || !g_vooc_chip->vops || !g_vooc_chip->vops->set_mcu_sleep) {
		return;
	} else {
...
{
	if (!g_vooc_chip || !g_vooc_chip->vops || !g_vooc_chip->vops->get_switch_gpio_val) {
		return 0;
	} else {
...
void oplus_vooc_turn_off_fastchg(void)
{
	...
	if (chip->vops && chip->vops->set_mcu_sleep) {
