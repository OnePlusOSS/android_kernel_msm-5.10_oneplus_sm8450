...
static void oplus_chg_batt_subs_callback(struct mms_subscribe *subs,
					 enum mms_msg_type type, u32 id)
{
	struct oplus_chg_chip *chip = subs->priv_data;
...
static int oplus_chg_subscribe_gauge_topic(struct oplus_chg_chip *chip)
{
	int rc;
	...
	snprintf(chip->batt_subs.name, TOPIC_NAME_MAX, "charger");
	chip->batt_subs.priv_data = chip;
	chip->batt_subs.callback = oplus_chg_batt_subs_callback;
	INIT_LIST_HEAD(&chip->batt_subs.list);
	rc = oplus_mms_subscribe(chip->gauge_topic, &chip->batt_subs);
	if (rc < 0) {
		chg_err("subscribe gauge topic error, rc=%d\n", rc);
		return rc;
	}
...
static void oplus_chg_gauge_update_work(struct work_struct *work)
{
	...
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_VOL_MAX,
				&data, false);
	chip->batt_volt_max = data.intval;
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_VOL_MIN,
				&data, false);
	chip->batt_volt_min = data.intval;
	chip->batt_volt = chip->batt_volt_max;
	...
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_CC,
				&data, false);
	chip->batt_cc = data.intval;
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_SOH,
				&data, false);
	chip->batt_soh = data.intval;
	oplus_mms_get_item_data(chip->gauge_topic, GAUGE_ITEM_RM,
				&data, false);
...
int oplus_chg_init(struct oplus_chg_chip *chip)
{
	...
	INIT_WORK(&chip->gauge_update_work, oplus_chg_gauge_update_work);
	rc = oplus_chg_subscribe_gauge_topic(chip);
	if (rc < 0)
		goto power_psy_reg_failed;
	...
	if (!chip->shortc_thread) {
		...
		rc = -EPROBE_DEFER;
		goto subscribe_gauge_topic_failed;
	}
...
subscribe_gauge_topic_failed:
	oplus_mms_unsubscribe(&chip->batt_subs);
...
struct oplus_chg_chip *get_oplus_chg_chip(void)
{
